...
class MultibancoPayment extends UnlimitPayment
{
    ...
    public function initialize($paymentAction, $stateObject)
    {
        try {
            $this->_helperData->log(
                self::MULTIBANCO_MESSAGE." - Multibanco: init prepare post payment", self::LOG_NAME
            );
            ...
            $this->_helperData->log(
                self::MULTIBANCO_MESSAGE." - Preference to POST", 'cardpay.log', $requestParams
            );
        } catch (Exception $e) {
            $this->_helperData->log(
                self::MULTIBANCO_MESSAGE." - There was an error retrieving the information to create the payment,
     ...
     * @throws LocalizedException
     */
    public static function isMultibancoPaymentMethod($order)
    {
        if (is_null($order)) {
            return false;
        }
