...
class RestClient
{
    ...
    private static function getConnection($uri, $method, $contentType, $extraParams = [])
    {
        if (!extension_loaded('curl')) {
            throw new UnlimitBaseException('cURL extension not found.'.
        ...
        if ((string)$contentType === self::CONTENT_TYPE) {
            ...
            if (function_exists('json_last_error')) {
                ...
                if ($json_error !== JSON_ERROR_NONE) {
                    throw new UnlimitBaseException("JSON Error [$json_error] - Data: $data");
                }
            }
        }
    ...
    protected static function exec($method, $url, $data, $contentType, $extraParams)
    {
        ...
        if ($apiResult === false) {
            throw new UnlimitBaseException(curl_error($connection));
        }
    ...
    private static function maskCardData($data)
    {
        if (isset($data['card_account']['card']['pan'])) {
            $data['card_account']['card']['pan'] = self::$_cpHelper->maskSensitiveInfo(
                $data['card_account']['card']['pan']
            );
        }
