...
class PaymentStatusHandler
{
    ...
    public function changePaymentStatus($observer, $statusTo, $expectedResponseStatus)
    {
        ...
        } elseif (isset($paymentResponse['recurring_data'])) {
            $apiStructure = 'recurring_data';
            $apiEndpoint = '/api/installments/';
        } else {
            throw new LocalizedException(__(self::ERROR_MESSAGE));
        }
    ...
    private function canPaymentStatusBeChanged($payment): bool
    {
        ...
        $paymentResponse = $payment['additional_information']['paymentResponse'];
        $isCardPayment = (isset($paymentResponse['payment_data']) && !empty($paymentResponse['payment_data']['id']));
        $isInstallment = (isset($paymentResponse['recurring_data']) && !empty($paymentResponse['recurring_data']['id']));
        return $isCardPayment || $isInstallment;
    }
